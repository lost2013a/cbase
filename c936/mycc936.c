...
static WCHAR ff_convert (WCHAR	chr, UINT dir);
static int utf8_to_unicode(unsigned char *utf8, unsigned char* unicode);

void utf8_to_gbk(unsigned char *src, unsigned char *dst)
{
	unsigned short temp; 
	unsigned char buf[2], nBytes;
	while(*src!=0)
	{
		nBytes= utf8_to_unicode(src, buf);
		if(nBytes == 1){
			*dst=buf[0];
			dst+=1;
		}
		else if(nBytes == 3){
			temp=(unsigned short)ff_convert((unsigned short)*(unsigned short*)buf,0);
			*(unsigned short*)dst=swap16(temp);
			dst+=2;
		}
		else
			break;
...
inline static unsigned char chr_to_hex(unsigned char chr)
{
...
unsigned int ucs2_to_gbk(unsigned char *src, unsigned char* dst)
{
	unsigned short temp; 
	unsigned char buf[2];
	unsigned char *begin= dst;
	{ 
		while(*src!=0)
		{
			buf[1]=chr_to_hex(*src++)*16;
			buf[1]+=chr_to_hex(*src++);
			buf[0]=chr_to_hex(*src++)*16;
			buf[0]+=chr_to_hex(*src++);
 			temp=(unsigned short)ff_convert((WCHAR)*(unsigned short*)buf,0);
			if(temp<0X80){
				if(temp == 0)	
					*dst=' ';
				else
...
static int utf8_to_unicode(unsigned char *utf8, unsigned char* unicode)
{
	...
	else
	{
		...
		{
			nBytes	= 3;
			unicode[1] = ((utf8[0] & 0x0f) <<4) + ((utf8[1] & 0x3c) >>2);
...
static WCHAR ff_convert (	/* Converted code, 0 means conversion error */
